if(obj_knight.knight_state == KNIGHT_STATE.CHARGE){
	...
	part_system_color(part_sys_shield, other.image_blend, 1);
	part_shield_play(_ps_bonk, x, y, _dir);
	...
	with(obj_knight){
		...
		var _hit_count = initial_charge_done ? hit_count : hit_count + charge_required_hits_initial;
		if(_hit_count % charge_required_hits == 0){
			// Feather ignore once GM2016
			knight_precharge();
		}
	}
